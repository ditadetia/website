...
if (!empty($_FILES["filegbr"]["tmp_name"]))
{
  ...
  if($jenis_gambar=="image/jpeg" || $jenis_gambar=="image/jpg" || $jenis_gambar=="image/png" || $jenis_gambar=="image/gif" || $jenis_gambar=="image/x-png")
  {
    $lampiran = $namafolder . basename($_FILES['filegbr']['name']);
      ...
      {
  $update = $conn->query("UPDATE tbl_berita SET judul='$judul', isi='$isi', gambar='$gambar' where id='$id'");
  if($update)
  {
    ?>
    ...
    <?php
      ...
      if(empty($_FILES["filegbr"]["tmp_name"]))
      {
        $update = $conn->query("UPDATE tbl_berita SET judul='$judul', isi='$isi' where id='$id'");
        if($update)
        {
          ?>
          <script language="javascript">
                      alert('Berhasil memperbarui Data Berita');
                      document.location="news.php";
                  </script>
                  <?php
        } else {
